...
def _upsert_run_metrics(supabase, run_id: str, metrics: Dict[str, float]) -> None:
    ...
    try:
        ...
        if "research_run_metrics_run_id_fkey" in error_text:
            _ensure_run_row(supabase=supabase, run_id=run_id, pipeline_name="stock_v3_subscription_alert")
            try:
                ...
                logger.warning(f"[SUB_ALERT_METRICS_RETRY_FAILED] error={str(inner)[:120]}")
                return
        logger.warning(f"[SUB_ALERT_METRICS_UPSERT_FAILED] error={error_text[:120]}")
...
def _ensure_run_row(supabase, run_id: str, pipeline_name: str) -> None:
    ...
    try:
        ...
        logger.warning(f"[SUB_ALERT_ENSURE_RUN_FAILED] error={str(e)[:120]}")
