...
**Q: GitHub Actions 的 crawler 已跑很久，但日志长时间没有新输出**

A: v0.3.1 已增加实时进度日志与非缓冲输出。请确认工作流使用：
- `python -u scripts/crawler.py`
- `PYTHONUNBUFFERED=1`

如果仍无输出，优先检查当前卡在的 Step（通常是 `Run crawler`）以及外部源可达性（Worker/Railway/Supabase）。
...
| 版本 | 日期 | 变更 |
|------|------|------|
| Phase 3.1 (v0.3.1) | 2026-02-19 | Crawler 实时进度日志、Actions 非缓冲输出、长任务可观测性增强 |
